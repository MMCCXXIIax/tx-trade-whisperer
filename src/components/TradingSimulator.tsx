...
const TradingSimulator: React.FC = () => {
  ...
  // Fetch positions using localStorage-based paper trading
  useEffect(() => {
    const loadPositions = () => {
      try {
    ...
    loadPositions();
    const interval = setInterval(loadPositions, 5000); // Refresh every 5s
    return () => clearInterval(interval);
  }, []);
  ...
  const executeTrade = async (side: 'BUY' | 'SELL') => {
    ...
    try {
      ...
      // Use localStorage for paper trading
      const trade = {
      ...
      // Update positions
      const openTrades = existingTrades.filter((t: any) => t.status === 'open');
      setPositions(openTrades);
    } catch (error) {
    ...
    try {
      ...
      const updatedTrades = existingTrades.map((trade: any) => {
        if (trade.symbol === symbol && trade.status === 'open') {
          ...
          return { ...trade, status: 'closed', closePrice: currentPrice, pnl };
        }
        return trade;
      });
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-sm font-bold trading-numbers">{JSON.parse(localStorage.getItem('paperTrades') || '[]').length}</div>
                  </div>
                </div>
              </CardContent>
            </Card>
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          ...
                          <div>
                            ...
                            <span className={`font-bold trading-numbers ml-1 ${pnl >= 0 ? 'text-tx-green' : 'text-tx-red'}`}>
                              {pnl >= 0 ? '+' : ''}${pnl.toFixed(2)}
                            </span>
                          </div>
                        </div>
